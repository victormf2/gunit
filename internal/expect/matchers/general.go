...
func NewGeneralMatcher(expected any) expect.GeneralMatcher {
...
func (g *generalMatcher) Match(actualValueInterface any) expect.MatchResult {
	actualValue := reflect.ValueOf(actualValueInterface)
	// Handle nil pointers
	if isNil(actualValue) {
		if isNil(g.expectedValue) {
			return expect.Matches() // Both nil, match
		}
		return expect.DoesNotMatch(fmt.Sprintf("Expected %v, but got nil", g.expectedValue.Interface()), nil)
	}
	if isNil(g.expectedValue) {
		return expect.DoesNotMatch(fmt.Sprintf("Expected nil, but got %v", actualValue.Interface()), nil)
	}
	...
	if !compatibleKinds(actualValue, g.expectedValue) {
		return expect.DoesNotMatch(fmt.Sprintf("Type mismatch: expected %s, got %s", g.expectedValue.Kind(), actualValue.Kind()), nil)
	}
